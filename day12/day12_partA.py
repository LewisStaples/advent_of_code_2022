...
def acceptable(distance_map, height_map, position_now, potential_new_position):
    ...
    if not math.isinf(
        distance_map[potential_new_position[0]][potential_new_position[1]]
    ):
        return False

    # Fail if new height is more than one greater than the old height
...
def set_distance_map(distance_map, old_position, new_position):
    distance_map[new_position[0]][new_position[1]] = (
        distance_map[old_position[0]][old_position[1]] + 1
    )
...
with open(input_filename) as f:
    ...
    for line_num, in_string in enumerate(f):
        ...
        if "S" in in_string:
            next_positions.append(np.array([line_num, in_string.find("S")]))
...
while True:
    ...
    for direction in [
        ...
        if acceptable(distance_map, height_map, position_now, potential_new_position):
            ...
            # if problem solved
            if height_map[potential_new_position[0]][potential_new_position[1]] == "E":
                print(
                    f"Fewest steps from S to E (part A) is: {distance_map[potential_new_position[0]][potential_new_position[1]]}\n"
                )
