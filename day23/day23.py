...
def make_moves(elf_map, round_number):
    ...
    for elf_location in elf_map:
        ...
        for i in range(4):
            ...
            proposed_destination = tuple(
                elf_location + proposal_fields["proposed_direction"]
            )
            required_vacancies = [
                tuple(elf_location + x) for x in proposal_fields["required_vacancies"]
            ]
            if required_vacancies_not_present(required_vacancies, elf_map):
                ...
                if proposed_destination in proposed_moves:
                    # If it's already there, mark that proposed_move
                    # as unreachable with a None (it's a flag)
                    proposed_moves[proposed_destination] = None
                else:
                    # This proposed_destination hasn't been seen before,
                    # so list it as a proposed_move
    ...
    return proposed_moves_pruned


def get_empty_tile_count(elf_map):
    margins = get_margins(elf_map)
    total_tile_count = (margins["bottom"] - margins["top"] + 1) * (
        margins["right"] - margins["left"] + 1
    )
...
def do_rounds(elf_map):
    ...
    round_number = 1
    while True:
        proposed_moves = make_moves(elf_map, round_number)
        # Solving part A / part 1:
        if round_number == 10:
            empty_tile_count = get_empty_tile_count(elf_map)
            print(f"The answer to part A is {empty_tile_count}\n")
        # No more moves will be made (part B / part 2):
        if set(proposed_moves.keys()) == set(proposed_moves.values()):
            return round_number
        round_number += 1
...
def solve_problem(input_filename):
    ...
    display_elf_map(elf_map, f"Elf locations in {input_filename}")
    round_number = do_rounds(elf_map)
    print(f"The answer to part B is {round_number}\n")
