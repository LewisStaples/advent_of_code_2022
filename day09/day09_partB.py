...
with open(input_filename) as f:
    ...
    for in_string in f:
        ...
        for i in range(distance):
            ...
            for i in range(1, 10):
                knots_locations[i] = get_new_tail_location(knots_locations[i-1], knots_locations[i])
            all_tail_points.add(tuple(knots_locations[-1]))

print(f'Total number of positions visited by tail (answer to B) is {len(all_tail_points)}\n')
